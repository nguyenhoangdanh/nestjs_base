...
export enum ErrorType {
  ...
  export class AppErrorFactory {
    ...
    static unauthorized(message = 'Unauthorized', details?: Record<string, any>): AppError {
      return AppError.from(new Error(message), 401).withDetail('errorType', ErrorType.UNAUTHORIZED);
    }
  
    static invalidCredentials(message = 'Invalid credentials'): AppError {
      return AppError.from(new Error(message), 400).withDetail('errorType', ErrorType.INVALID_CREDENTIALS);
    }
