...
export class CleanupJobService {
  ...
  async cleanupInactiveSessions() {
    ...
    try {
      ...
      this.logger.error(
        `Error cleaning up inactive sessions: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async cleanupRedisCache() {
    ...
    try {
      ...
      this.logger.error(
        `Error cleaning up Redis cache: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async purgeDeletedUsers() {
    ...
    try {
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        await tx.biometricData.deleteMany({
      ...
      this.logger.error(
        `Error purging deleted users: ${error.message}`,
        error.stack,
      );
    }
  }
