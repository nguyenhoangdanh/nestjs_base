...
export class PartitionJobService {
  ...
  async createNextWeekPartition() {
    ...
    try {
      ...
      this.logger.log(`Created partition ${partitionName} for dates ${startDateStr} to ${endDateStr}`);
    } catch (error) {
      this.logger.error(`Error creating next week partition: ${error.message}`, error.stack);
    }
  }
  ...
  async cleanupOldPartitions() {
    ...
    try {
      ...
      this.logger.error(`Error cleaning up old partitions: ${error.message}`, error.stack);
    }
  }
